...
The starting point is the establishment of a forward model. Let $u \in \mathcal{U}(\mathbb{R}^d)$ be the imaged continuous, unknown physical object of interest embedded in a $d$-dimensional space. The process of signal transmission and subsequent digital recording can be encapsulated by the generic mapping defined as:
\begin{equation}
...
\begin{itemize}
    \item $\mathcal{H}: \mathcal{U}(\mathbb{R}^d) \to \mathcal{Y}(\mathbb{R}^{d'})$ models the physics between the object and the signal to be recorded by the detector;
    \item $\mathcal{S}: \mathcal{Y}(\mathbb{R}^{d'}) \to \mathbb{R}^M$ models the integration on the detector; 
